...
class ResultValidator:
    ...
    def validate_and_normalize(
        ...
        if row_count > settings.MAX_RESULT_ROWS:
            raise ResultValidationError(
                f"Result row count {row_count} exceeds maximum allowed limit of {settings.MAX_RESULT_ROWS}."
            )
        ...
        # 3. Column validation
        if expected_columns:
            # All expected columns should be present in raw_columns (case-insensitive check)
            returned_cols_lower = {c.lower() for c in raw_columns}
            for exp in expected_columns:
                # Handle aliases or table qualifications (e.g., 't.col' -> 'col')
    ...
    def _normalize_cell(
        ...
        if isinstance(value, float):
            ...
            return self._apply_sanity_bounds(value, col_name, row_idx, metric_unit)
        ...
        if isinstance(value, Decimal):
            ...
            return self._apply_sanity_bounds(float_val, col_name, row_idx, metric_unit)
    ...
    def _apply_sanity_bounds(
        ...
        # Percentage sanity checks: [0, 100]
        if metric_unit == "percentage" or "percent" in col_lower or "pct" in col_lower:
